    ...
    return BlocConsumer<StartAppCubit, StartAppState>(
      listener: (context, state) {
        ...
        //    CacheHelper.getData(key: 'isSubscribed')
            isProviderSubscribed == false && CacheHelper.getData(key: 'status') == 'service_provider') {
              navigateForwardReplace(PackagesScreen());
            } else {
              isBioActiveForUser==true?_authenticateWithBiometrics():
              navigateForwardReplace(MainScreenNavigation());
        ...
        else if (state is GetAccountTypeDataErrorState || state is GetBoardingDataErrorState) {
