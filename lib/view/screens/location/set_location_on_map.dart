            ...
            leading: Padding(
              ...
              child: BackButtonWidget(
                onTap: (){
                 ...
                 print('<<<<<<<<<<<<<<<<<${locationCubit.regionId}>>>>>>>>>>>>>>>>>');
                  print('<<<<<<<<<<<<<<<<<${locationCubit.cityId}>>>>>>>>>>>>>>>>>');
                },
              ),
            ),
            //  leading: BackButtonWidget(radius: 2),
            actions: [
              state is SetLocationFromMapLoadingState?const SizedBox():
              IconButton(
                  onPressed: () {
                    widget.locationName.text = locationCubit.addressFromMap;
                    widget.cityName?.text = locationCubit.cityTextController.text;
                    widget.areaName?.text = locationCubit.districtTextController.text;
                    widget.regionName?.text=locationCubit.regionTextController.text;
                        ...
                        GoogleMap(
                          ...
                          onCameraIdle: () async {
                            if(widget.isEdit==true){
                              await  locationCubit.getLocationFromMap(cameraPositions,true);
                            }else{
                              await  locationCubit.getLocationFromMap(cameraPositions,false);
                            }
