...
class CompletePriceOfferPayment extends StatefulWidget {
  final Function onSuccess, onError;
  final String url;

  const CompletePriceOfferPayment({
    Key? key,
...
class _CompleteAdPaymentState extends State<CompletePriceOfferPayment> {
  void listenChatChannel() {
    LaravelEcho.instance
        .channel('payment-channel')
        .listen('.new-price-offer-paid-${userId}', (e) {
      if (e is PusherEvent) {
        if (e.data != null) {
          jsonDecode(e.data!);
          ...
          context
              .read<SubscribeCubit>()
              .handelPriceOfferResponsePayment(jsonDecode(e.data!));
        }
        print('????${jsonDecode(e.data!)}');
      }
    }).error((error) {
          ...
          child: Scaffold(
            ...
            body: Container(
              child: subscribeCubit.loading
                  ...
                  subscribeCubit.paymentPriceServiceModel.paymentDetails
                              ?.message ==
                          'you have paid for service successfully'
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              PaymentDetailsWidget(
                                transaction: subscribeCubit
                                        .paymentPriceServiceModel
                                        .paymentDetails
                                        ?.data
                                        ?.transaction ??
                                    '',
                                orderNumber: subscribeCubit
                                        .paymentPriceServiceModel
                                        .paymentDetails
                                        ?.data
                                        ?.orderNumber ??
                                    '',
                                type: LocaleKeys.serviceFees.tr(),
                                price: subscribeCubit.paymentPriceServiceModel
                                        .paymentDetails?.data?.price
                                        .toString() ??
                                    '',
                                isOffer: true,
                                bidDuration: subscribeCubit
                                        .paymentPriceServiceModel
                                        .paymentDetails
                                        ?.data
                                        ?.payDate ??
